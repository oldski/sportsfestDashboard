...
import {
  CalendarCheckIcon, ClipboardListIcon, MegaphoneIcon, UsersIcon
} from "lucide-react";
import { EventInfoBar } from '~/components/organizations/slug/dashboard/event-info-bar';
        ...
        <PageHeader>
          ...
          {currentEventYear.id && (
            <PageSecondaryBar className="bg-muted/30">
              <EventInfoBar currentEventYear={currentEventYear} />
            </PageSecondaryBar>
          )}
        </PageHeader>
        <PageBody>
          <div className="mx-auto w-full space-y-6 p-2 sm:space-y-8 sm:p-6">
            <Card className="border-none shadow-none pb-0">
              <CardHeader className="px-2 md:px-0">
                <CardTitle className="text-xl font-semibold">Welcome to {stats.currentEventYear.name}, {stats.organizationName}!</CardTitle>
                <CardDescription>Start Building Your Team for FREE! When you are ready to pay, then Register.</CardDescription>
              </CardHeader>
              <CardContent className="px-2 md:px-0 space-y-2 text-sm leading-relaxed">
                <div className="flex flex-col lg:flex-row justify-between gap-4">
                  <div className="space-y-4">
                    <p className="text-md leading-relaxed">
                      Get ready to rally your coworkers and build the ultimate team-building experience. When you are ready to pay, then Register.  From your dashboard, you
                      can:
                    </p>
