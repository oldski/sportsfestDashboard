...
export function DragOverlayPlayer({ player }: DragOverlayPlayerProps) {
  const GenderIcon = player.gender === 'male' ? UserCheck : Users;

  return (
    <div
      className={cn(
        'flex items-center gap-3 p-3 bg-background border-2 rounded-lg select-none',
        'shadow-2xl border-primary/30 bg-background/95 backdrop-blur-sm cursor-grabbing',
        'min-w-0 max-w-none'
      )}
      style={{ 
        width: 'max-content',
